...
def generate_sample_data_with_pose(args, nusc, trajectory_datas):
    ...
    for trajectory_data in tqdm(trajectory_datas):
        ...
        for sensor in sensor_modalities:
            if sensor not in sample_first["data"]:
                continue
            sample_first_data_token = sample_first["data"][sensor]
            sample_data_count_cycle=sample_data_count
            
            sample_first_data = nusc.get("sample_data", sample_first_data_token)
            new_sample_first_data = copy.deepcopy(sample_first_data)
            new_sample_first_data["token"] = generate_token(new_sample_first_data["token"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
            new_sample_first_data["sample_token"] = generate_token(new_sample_first_data["sample_token"],f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
            new_sample_first_data["timestamp"] = first_time_stamp
            if sample_data_count_cycle == 0:
                new_sample_first_data["prev"] = ''
            else:
                new_sample_first_data["prev"] = generate_token(new_sample_first_data["prev"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
            new_sample_first_data["next"] = generate_token(new_sample_first_data["next"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
            new_sample_first_data["ego_pose_token"] = generate_token(new_sample_first_data["ego_pose_token"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
            new_sample_data_list.append(new_sample_first_data)

            # Update ego pose token
            ego_pose_token = sample_first_data["ego_pose_token"]
            ego_pose = nusc.get("ego_pose", ego_pose_token)
            new_ego_pose = copy.deepcopy(ego_pose)
            new_ego_pose["token"] = generate_token(new_ego_pose["token"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
            new_ego_pose["translation"] = trajectory_data["ego_translation"][sample_data_count_cycle].tolist()
            new_ego_pose["rotation"] = trajectory_data["ego_rotation"][sample_data_count_cycle].tolist()

            new_ego_pose_list.append(new_ego_pose)
            
                
        sample_data_count = sample_data_count+1#sampletokenABBB
        sample_token=first_sample_token
        for i in range(18):  # Process 21 samples (0-20 inclusive)
            
            # Process each of the next samples in the same way as the first sample
            sample = nusc.get("sample", sample_token)
            next_sample_token = sample["next"]
            # if i==18:
            #     print("a")
            for sensor in sensor_modalities:
                # if i==18 and sensor=="CAM_FRONT_RIGHT":
                #     print("b")
                if sensor not in sample["data"]:
                    continue
                sample_data_token = sample["data"][sensor]
                sample_data_count_cycle = sample_data_count
                sample_data_the_first_key = nusc.get("sample_data", sample_data_token)
                sample_data_token=sample_data_the_first_key['next']
                while True:
                    sample_data= nusc.get("sample_data", sample_data_token)
                    new_sample_data = copy.deepcopy(sample_data)
                    new_sample_data["token"] = generate_token(new_sample_data["token"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
                    new_sample_data["sample_token"] = generate_token(new_sample_data["sample_token"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
                    if args.dataset_type!= 'oracle':
                        if nusc.get("sample_data", sample_data['next'])["sample_token"] != next_sample_token:
                            new_sample_data['timestamp']=int(first_time_stamp + (sample_data_count+5) / 12.0 * 1000000)
                        else:
                            new_sample_data["timestamp"] = int(first_time_stamp + sample_data_count_cycle / 12.0 * 1000000)
                    # Handle prev and next links
                    if new_sample_data["prev"] != '':
                        new_sample_data["prev"] = generate_token(new_sample_data["prev"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
                    if new_sample_data["next"] != '':
                        new_sample_data["next"] = generate_token(new_sample_data["next"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
                    if i==17 and (sample_data["next"]=="" or nusc.get("sample_data", sample_data['next'])["sample_token"] != next_sample_token):
                        new_sample_data["next"] = ''
                    # Update ego pose token
                    new_sample_data["ego_pose_token"] = generate_token(new_sample_data["ego_pose_token"], f"_collision_{collision}_adv_{adv_id}_startFrameIndex_{start_frame_index}")
                    new_sample_data_list.append(new_sample_data)
                    
                    # Create corresponding ego pose entry
                    ...
                    new_ego_pose_list.append(new_ego_pose)
                    sample_data_count_cycle += 1
                    
                    # Move to next sample data
                    sample_data_token = sample_data["next"]
                    if sample_data_token == "" or nusc.get("sample_data", sample_data_token)["sample_token"] != next_sample_token:
                        break
                    #sample_data_count_cycle6
                    if sample_data_count_cycle-sample_data_count>=6:
                        sample_data_count_cycle-=1
            
            # sample_data_count = sample_data_count_cycle-1 #
            sample_data_count = sample_data_count+6
...
def parse_args():
    ...
    parser.add_argument('--dataset_type', type=str, default="oracle", help='oracle(token)')
